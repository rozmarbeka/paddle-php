...
class Api
{
    ...
    public function __construct($vendor_id = null, $vendor_auth_code = null, $sandbox_mode = true, $timeout = null)
    {
        if ($vendor_id && $vendor_auth_code) {
            $this->set_vendor_credentials($vendor_id, $vendor_auth_code);
        }

        $this->base_url = $sandbox_mode ? self::SANDBOX_BASE_URL : self::BASE_URL;
    ...
    private function http_call($path, $method, $parameters = array())
    {
        ...
        if ($arr_api_response['success'] !== true) {
            throw new \Exception('API error: ' . $arr_api_response['error']['code'] . ' ' . @$arr_api_response['error']['message'], $arr_api_response['error']['code']);
        }
     ...
     * @throws \Exception
     */
    public function refund($order_id, $amount = null, $reason = null)
    {
        $data = array();
        ...
        if ($reason) {
            $data['reason'] = Filters::filter_title($reason);
        }

        return $this->http_call('/payment/refund', 'POST', $data);
    }
    ...
    public function generate_custom_product_pay_link($title, $price, $image_url, $webhook_url, array $optional_arguments)
    {
        ...
        if (isset($optional_arguments['prices'])) {
            $data['prices'] = $optional_arguments['prices'];
        }
        ...
        if (isset($optional_arguments['discountable'])) {
            throw new \InvalidArgumentException(self::ERR_316, 316);
        }
        // coupon_code (forbidden)
        if (isset($optional_arguments['coupon_code'])) {
            throw new \InvalidArgumentException(self::ERR_317, 317);
        }
